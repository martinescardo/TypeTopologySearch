...
def write_emacs_index(rows, out, sourcedir):
    ...
    for r in sorted(rows, key=lambda r: sortkey(r["name"])):
        ...
        lines.append("\t".join([
            ...
            field(r.get("sig") or ""), field(r.get("scope_text") or "")]))
    header = ["# TypeTopology definitions index for programs, tab-separated. "
              "Columns: name, module (with any inner",
              "# submodule), module alone (what \"open import\" wants), "
              "source file relative to the source directory,",
              "# source line, use count, signature where known, any "
              "hypothesis taken by an enclosing module.",
              "# Every line below is data, none of them this header -- a "
              "reader just drops lines starting with \"#\"",
              f"# and splits the rest on tabs. {len(lines)} definitions. "
              "Regenerated by 'agda-index.py --emacs-index';",
              "# not kept automatically in sync, so re-run first."]
    open(f"{out}/Definitions.tsv", "w", encoding="utf-8").write(
        "\n".join(header + lines) + "\n")
...
def main():
    ...
    people = []
    if not a.no_html:
        people = people_of(a.readme, body) if os.path.exists(a.readme) else []
    ...
    if a.emacs_index:
        write_emacs_index(rows, a.out, a.source)
