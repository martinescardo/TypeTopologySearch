...
def write_emacs_index(rows, out, sourcedir, people, concepts, paras):
    ...
    #
    # Contributors, concepts (from the same README-derived list and
    # concepts.tsv the search page shows), and comments (one row per prose
    # paragraph, see paragraphs_of) are rows here too, told apart by the
    # trailing "kind" column ("def", "person", "concept", or "comment") --
    # none of the three has a module, file, line, or signature of its own
    # the way a definition does, so those columns are simply empty for a
    # person or concept; the use count column is repurposed for a person
    # or concept to hold how many modules mention them, and the assumes
    # column to hold which ones, semicolon-separated (a module's dotted
    # name never contains one). A comment's own module -- exactly one,
    # unlike a person's or concept's -- goes in BOTH the module columns
    # (so "in PATH" scopes it exactly like a definition) AND assumes (so
    # typetopology-search.el's jump-to-mention, shared with people and
    # concepts, can read it the same way), since a paragraph has no file,
    # line, or signature of its own either.
    src_of = {}
    def source_file(mod):
    ...
    for name, mods in concepts:
        entries.append((name, [name, "", "", "", "0", str(len(mods)),
                                "", ";".join(mods), "concept"]))
    ncoms = 0
    for m, ps in paras.items():
        for p in ps:
            entries.append((p, [field(p), m, m, "", "0", "0", "", m, "comment"]))
            ncoms += 1
    ...
    header = [f"# {REPO_NAME} definitions, contributors, concepts, and "
              ...
              "module (a contributor's, concept's, or comment's: which",
              "# module(s) mention them, semicolon-separated -- always "
              "exactly one for a comment), and kind (\"def\", \"person\",",
              "# \"concept\", or \"comment\" -- none of the last three has "
              "a source file, line, or signature of its own).",
...
function search(){
  ...
  // A contributor is stored under a key with the accents taken off, so that
  // a file writing Escardó and a file writing Escardo name the same person.
  // The query has to be folded the same way before it is compared with that
  // key, or a reader who spells Opršal as he does reaches nobody.
  const unaccent=s=>s.normalize("NFKD").replace(/\p{M}/gu,"");
  const pterms=needle.split(/\s+/).map(w=>unaccent(w))
                     .map(w=>({w:w, rx:wildcard(w,whole)}));
  let ppl=PEOPLE.filter(p=>pterms.every(t=>has(p[1],t)));
...
def main():
    ...
    if a.emacs_index:
        write_emacs_index(rows, a.out, a.source, people, concepts, paras)
